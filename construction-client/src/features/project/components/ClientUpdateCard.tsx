...
import type { Attachment, ProjectReply, ProjectUpdate } from "@features/admin";
...
type ReplyTarget = { type: "update"; parentReplyId?: string } | null;

const ClientUpdateCard: React.FC<ClientUpdateCardProps> = ({ update, onOpenImage, clientName, onReply }) => {
  ...
  return (
    <div className="border-l-4 border-emerald-500 bg-white rounded-lg p-6 shadow-sm hover:shadow-md transition">
      ...
      <div className="mt-4 pt-3 border-t border-gray-100 flex flex-col gap-3">
        ...
        {update.createdBy === "admin" && onReply && (
          <button
            type="button"
            onClick={() => startReply({ type: "update" })}
            className="px-3 py-1.5 text-xs font-semibold text-emerald-800 bg-emerald-100 hover:bg-emerald-200 rounded-lg w-fit"
          >
            Reply
          </button>
        )}
